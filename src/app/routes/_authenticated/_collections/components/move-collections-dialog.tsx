...
function CollectionTreeNode({
  ...
  const isChildOfBeingMoved = collectionsToMove.some((c) =>
    collection.breadcrumbs.some((b) => b.id === c.id),
  )
...
export function MoveCollectionsDialog({
  ...
  const childCollectionsForMoveKey = (collectionId?: string) =>
    collectionId
      ? ['childCollectionsForMove', collectionId]
      : ['childCollectionsForMove']
  ...
  const selectionHasTopLevelParent = collectionsToMove.some(
    (c) => c.parentId === topLevelCollectionId,
  )
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh]">
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleMove}
            disabled={
              !selectedCollectionId || moveCollectionsMutation.isPending
            }
          >
