...
function ModifyOrderPage() {
  ...
  function handleAdjustLine({
    ...
    if (lineId.startsWith('added-')) {
      ...
      setModifyOrderInput((prev) => {
        const existing = (prev.adjustOrderLines ?? []).find(
  ...
  function handleSetShippingMethod({
    shippingMethodId,
  }: {
  ...
  function computePendingOrder(input: ModifyOrderInput): Order | null {
    ...
    const lines = entity.lines.map((line) => {
      const adjust = input.adjustOrderLines?.find(
      ...
      return adjust
        ? {
            ...
            customFields: (adjust as any).customFields,
          }
        : line
    })
  ...
  const shippingAddress =
    modifyOrderInput.updateShippingAddress ?? entity.shippingAddress
  const billingAddress =
    modifyOrderInput.updateBillingAddress ?? entity.billingAddress
      ...
      <PageActionBar>
        <PageActionBarRight>
          <Button
            type="button"
            variant="secondary"
            onClick={handleCancelModificationClick}
          >
      ...
      <PageLayout>
        ...
        >
          <OrderModificationSummary
            ...
            eligibleShippingMethods={
              eligibleShippingMethods?.eligibleShippingMethodsForDraftOrder?.map(
                (m) => ({
                  id: m.id,
                  name: m.name,
                }),
