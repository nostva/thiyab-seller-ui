...
export function FulfillmentDetails({
  ...
  fulfillment,
  onSuccess,
}: Readonly<FulfillmentDetailsProps>) {
  ...
  const transitionFulfillmentMutation = useMutation({
    ...
    onSuccess: (
      result: ResultOf<typeof transitionFulfillmentToStateDocument>,
    ) => {
      ...
      if (fulfillment.__typename === 'Fulfillment') {
        ...
        toast.error(
          fulfillment.message ?? i18n.t('Failed to update fulfillment state'),
        )
      }
    },
    onError: (error) => {
  ...
  return (
    <div className="space-y-1 p-3 border rounded-md">
      <div className="space-y-1">
        <LabeledData
          label={<Trans>Fulfillment ID</Trans>}
          value={fulfillment.id.slice(-8)}
        />
        ...
        {fulfillment.trackingCode && (
          <LabeledData
            label={<Trans>Tracking code</Trans>}
            value={fulfillment.trackingCode}
          />
        )}
        <LabeledData
          ...
          <Collapsible>
            ...
            <CollapsibleContent className="mt-2 space-y-1">
              {fulfillment.lines.map((line) => {
                ...
                const productName =
                  orderLine?.productVariant?.name ?? 'Unknown product'
