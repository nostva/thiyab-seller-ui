...
import { FormControl, FormField } from '@/components/ui/form.js'
...
export type OrderFragment = NonNullable<
  ResultOf<typeof orderDetailDocument>['order']
>
...
export function OrderModificationPreviewDialog({
  ...
  const confirmMutation = useMutation({
    mutationFn: (input: ModifyOrderInput) =>
      api.mutate(addCustomFields(modifyOrderDocument))({ input }),
  })
  ...
  useEffect(() => {
    if (open) {
      ...
      if (
        ...
        JSON.stringify(lastInputRef.current) !==
          JSON.stringify(modifyOrderInput)
      ) {
  ...
  const previewOrder =
    previewMutation.data?.modifyOrder?.__typename === 'Order'
      ? previewMutation.data.modifyOrder
      : null
  const error =
  ...
  // Price difference
  const priceDifference = previewOrder
    ? previewOrder.totalWithTax - orderSnapshot.totalWithTax
    : 0
  ...
  const isRefundComplete =
    priceDifference > 0 || totalRefundAmount >= Math.abs(priceDifference)
                              ...
                              <CardHeader className="">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    const availableAmount = payment.amount
                                    ...
                                    const remainingFromThisPayment =
                                      availableAmount - currentRefundAmount
                                    ...
                                    if (amountToAdd > 0) {
                                      refundForm.setValue(
                                        `payments.${payment.id}`,
                                        currentRefundAmount + amountToAdd,
                                      )
                                    }
                                  }}
                                >
                ...
                <div className="max-w-l mb-2">
                  {priceDifference > 0 && (
                    <Alert variant="destructive">
                      ...
                      <AlertDescription>
                        <Trans>
                          An additional payment of {formattedDiff} will be
                          required.
                        </Trans>
                      </AlertDescription>
                    </Alert>
