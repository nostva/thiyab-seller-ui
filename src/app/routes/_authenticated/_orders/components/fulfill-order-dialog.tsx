...
export function FulfillOrderDialog({
  ...
  const initializeFulfillmentQuantities = () => {
    ...
    order.lines.forEach((line) => {
      const fulfillCount = getFulfillableCount(
        line,
        globalSettingsData.globalSettings.trackInventory,
      )
  ...
  const getFulfillableCount = (
    ...
    return effectiveTrackInventory
      ? Math.min(unfulfilledCount, stockOnHand)
      : unfulfilledCount
  }
  ...
  const canSubmit = (): boolean => {
    ...
    const fulfillmentQuantityIsValid = Object.values(
      fulfillmentQuantities,
    ).every(({ fulfillCount, max }) => fulfillCount <= max && fulfillCount >= 0)
  ...
  const selectedHandler = fulfillmentHandlers?.find(
    (h) => h.code === form.watch('handler.code'),
  )
      ...
      <Dialog open={open}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              <Trans>
                Select quantities to fulfill and configure the fulfillment
                handler
              </Trans>
            </DialogDescription>
          </DialogHeader>
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          <Trans>
                            {quantity.max} of {line.quantity} available to
                            fulfill
                          </Trans>
                        </div>
                      </div>
