...
import { getColumnVisibility } from '@/components/data-table/data-table-utils.js'
import { DataTable } from '@/components/data-table/data-table.js'
import { useGeneratedColumns } from '@/components/data-table/use-generated-columns.js'
  ...
  return {
    ...
    fulfillmentLines: {
      cell: ({ row }: { row: any }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <JsonEditor
              data={row.original.fulfillmentLines}
              viewOnly
              rootFontSize={12}
            />
          </DropdownMenuContent>
    ...
    proratedUnitPrice: {
      cell: ({ row }: { row: any }) => (
        <Money
          currencyCode={currencyCode}
          value={row.original.proratedUnitPrice}
        />
      ),
    },
    proratedUnitPriceWithTax: {
      cell: ({ row }: { row: any }) => (
        <Money
          currencyCode={currencyCode}
          value={row.original.proratedUnitPriceWithTax}
        />
      ),
    },
    linePrice: {
    ...
    discountedLinePrice: {
      cell: ({ row }: { row: any }) => (
        <Money
          currencyCode={currencyCode}
          value={row.original.discountedLinePrice}
        />
      ),
    },
    lineTax: {
    ...
    discountedLinePriceWithTax: {
      cell: ({ row }: { row: any }) => (
        <Money
          currencyCode={currencyCode}
          value={row.original.discountedLinePriceWithTax}
        />
      ),
    },
  }
...
export function OrderTable({ order, pageId }: Readonly<OrderTableProps>) {
  ...
  const customizeColumns = useMemo(
    () => createCustomizeColumns(currencyCode),
    [currencyCode],
  )
