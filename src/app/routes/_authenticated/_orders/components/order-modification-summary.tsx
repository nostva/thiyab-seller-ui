...
export function OrderModificationSummary({
  ...
  const originalLineMap = new Map(
    originalOrder.lines.map((line) => [line.id, line]),
  )
  ...
  const adjustedLines = (modifyOrderInput.adjustOrderLines ?? [])
    .map(
      (
        adj: NonNullable<ModifyOrderInput['adjustOrderLines']>[number] & {
          customFields?: any
        },
      ) => {
        ...
        if (
          ...
          (adj.quantity !== orig.quantity ||
            JSON.stringify(adj.customFields) !==
              JSON.stringify((orig as any).customFields))
        ) {
          return {
  ...
  const addedCouponCodes = Array.from(modifiedCoupons).filter(
    (code) => !originalCoupons.has(code),
  )
  const removedCouponCodes = Array.from(originalCoupons).filter(
    (code) => !modifiedCoupons.has(code),
  )

  // Shipping method change detection
  const originalShippingMethodId =
    originalOrder.shippingLines?.[0]?.shippingMethod?.id
  ...
  if (
    ...
    modifiedShippingMethodId !== originalShippingMethodId
  ) {
    shippingMethodChanged = true
    newShippingMethodName =
      eligibleShippingMethods.find((m) => m.id === modifiedShippingMethodId)
        ?.name || modifiedShippingMethodId
  }
