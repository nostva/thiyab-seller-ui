...
const updateOrderNoteDocument = graphql(`
  mutation UpdateOrderNote($noteId: ID!, $note: String!, $isPublic: Boolean!) {
    updateOrderNote(
      input: { noteId: $noteId, note: $note, isPublic: $isPublic }
    ) {
      id
    }
  }
...
export interface UseOrderHistoryResult {
  historyEntries: NonNullable<
    ResultOf<typeof orderHistoryDocument>['order']
  >['history']['items']
...
export function useOrderHistory({
  ...
  const historyEntries =
    data?.pages
      .flatMap((page) => page.order?.history?.items)
      .filter((x) => x != null) ?? []
