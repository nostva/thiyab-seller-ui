...
import {
  cancelJobDocument,
  jobListDocument,
  jobQueueListDocument,
} from './job-queue.graphql.js'
...
function JobQueuePage() {
  ...
  const currentInterval = REFRESH_INTERVALS.find(
    ...
    <ListPage
      ...
      customizeColumns={{
        ...
        data: {
          ...
          cell: ({ row }) => (
            <PayloadDialog
              ...
              description={
                <Trans>The data that has been passed to the job</Trans>
              }
              trigger={
        ...
        queueName: {
          header: 'Queue',
          cell: ({ row }) => (
            <span className="font-mono">{row.original.queueName}</span>
          ),
        },
        result: {
          header: 'Result',
          ...
          cell: ({ row, table }) => {
            const cancelJobMutation = useMutation({
              mutationFn: (jobId: string) =>
                api.mutate(cancelJobDocument, { jobId }),
              onSuccess: () => {
                refreshRef.current()
              },
            })
                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() =>
                            cancelJobMutation.mutate(row.original.id)
                          }
          ...
          <DropdownMenuContent align="end">
            {REFRESH_INTERVALS.map((interval) => (
              <DropdownMenuItem
                ...
                className={
                  refreshInterval === interval.value ? 'bg-accent' : ''
                }
              >
                {interval.label}
              </DropdownMenuItem>
