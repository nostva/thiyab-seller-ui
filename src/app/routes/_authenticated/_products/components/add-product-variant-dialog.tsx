...
export function AddProductVariantDialog({
  ...
  const [duplicateVariantError, setDuplicateVariantError] = useState<
    string | null
  >(null)
    ...
    (values: FormValues) => {
      ...
      const selectedOptions = Object.entries(values.options)
        .map(([groupId, optionId]) => {
          const group = productData.product?.optionGroups.find(
            (g) => g.id === groupId,
          )
  ...
  useEffect(() => {
    ...
    checkForDuplicateVariant,
    generateNameFromOptions,
    form,
  ])
          ...
          >
            ...
            <DialogFooter className="flex flex-col items-end gap-2">
              ...
              <Button
                type="submit"
                disabled={
                  createProductVariantMutation.isPending ||
                  !!duplicateVariantError
                }
              >
