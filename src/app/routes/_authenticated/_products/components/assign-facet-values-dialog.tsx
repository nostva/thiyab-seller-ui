...
import {
  FacetValue,
  FacetValueSelector,
} from '@/components/shared/facet-value-selector.js'
...
export function AssignFacetValuesDialog({
  ...
  const [removedFacetValues, setRemovedFacetValues] = useState<Set<string>>(
    new Set(),
  )
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: () => {
      toast.error(
        `Failed to update facet values for ${entityIds.length} ${entityType}`,
      )
    },
  })
  ...
  const handleAssign = () => {
    if (selectedValues.length === 0 && !facetValuesRemoved) {
      toast.error(
        'Please select at least one facet value or make changes to existing ones',
      )
      return
    }
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-hidden flex flex-col">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {displayFacetValues.map((facetValue) => (
                                <FacetValueChip
                                  ...
                                  removable={true}
                                  onRemove={() =>
                                    removeFacetValue(entity.id, facetValue.id)
                                  }
                                />
                              ))}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              (selectedValues.length === 0 && !facetValuesRemoved) || isPending
            }
          >
            <Trans>Update</Trans>
          </Button>
