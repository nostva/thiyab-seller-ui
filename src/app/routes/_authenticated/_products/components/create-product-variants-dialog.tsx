...
} from './create-product-variants.js'
...
export function CreateProductVariantsDialog({
  ...
  const [variantData, setVariantData] = useState<VariantConfiguration | null>(
    null,
  )
  ...
  async function handleCreateVariants() {
    ...
    try {
      ...
      await createProductVariantsMutation.mutateAsync({
        input: variantsToCreate,
      })
      setOpen(false)
      onSuccess?.()
    } catch (error) {
            ...
            >
              ...
              createProductVariantsMutation.isPending ? (
                ...
                <Trans>
                  Create{' '}
                  {variantData
                    ? Object.values(variantData.variants).filter(
                        (v) => v.enabled,
                      ).length
                    : 0}{' '}
                  variants
