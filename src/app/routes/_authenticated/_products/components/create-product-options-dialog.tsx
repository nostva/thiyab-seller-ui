...
const formSchema = z.object({
  ...
  existingVariantOptionIds: z
    .array(z.string())
    .min(1, 'Must select an option for the existing variant'),
})
...
export function CreateProductOptionsDialog({
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const selectedOptionIds = values.existingVariantOptionIds.map(
        (optionName, index) => {
          const group = allOptionGroups[index]
          const option = group.options.find((opt) => opt.name === optionName)
          if (!option) {
            throw new Error(
              `Option "${optionName}" not found in group "${group.name}"`,
            )
          }
          return option.id
        },
              ...
              <div className="space-y-4 p-4 border rounded-lg">
                ...
                {form.watch('optionGroups').map((group, groupIndex) => (
                  <FormFieldWrapper
                    ...
                    label={
                      group.name || <Trans>Option group {groupIndex + 1}</Trans>
                    }
                    render={({ field }) => (
                      <select
                        className="w-full p-2 border rounded-md"
            ...
            <DialogFooter>
              <Button
                ...
                disabled={
                  ...
                  (productData?.product?.variants[0] &&
                    form
                      .watch('existingVariantOptionIds')
                      .some((value) => !value))
                }
              >
                <Trans>Create options</Trans>
