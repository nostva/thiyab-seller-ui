...
const variantSchema = z.object({
  ...
  stock: z
    .string()
    .refine((val) => !isNaN(Number(val)) && parseInt(val, 10) >= 0, {
...
export function CreateProductVariants({
  ...
  const { data: stockLocationsResult } = useQuery({
    ...
    queryFn: () =>
      api.query(getStockLocationsDocument, { options: { take: 100 } }),
  })
    ...
    const subscription = form.watch((value, { name, type }) => {
      if (value?.optionGroups) {
        ...
        const validOptionGroups = value.optionGroups
          ...
          .filter(
            (group) =>
              typeof group.name === 'string' && Array.isArray(group.values),
          )
          .map((group) => ({
            name: group.name,
            values: (group.values || [])
              ...
              .filter(
                (v) => typeof v.value === 'string' && typeof v.id === 'string',
              )
              .map((v) => ({
                value: v.value,
                id: v.id,
          ...
          ) as VariantConfiguration['optionGroups']
  ...
  return (
    <FormProvider {...form}>
      {optionGroups.map((group, index) => (
        <div
          key={group.id}
          className="grid grid-cols-[1fr_2fr_auto] gap-4 mb-6 items-start"
        >
          <div>
            <FormField
...
function generateVariants(groups: OptionGroupForm[]): GeneratedVariant[] {
  ...
  const validGroups = groups.filter(
    (group) => group.name && group.values && group.values.length > 0,
  )
