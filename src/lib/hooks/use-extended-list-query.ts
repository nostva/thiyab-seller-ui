...
export function useExtendedListQuery<T extends DocumentNode>(listQuery: T) {
  ...
  const listQueryExtensions =
    pageId && blockId ? getListQueryDocuments(pageId, blockId) : []
      ...
      if (error && !hasShownError.current) {
        ...
        if (
          error.message.includes(
            'Extension query must have at least one top-level field',
          )
        ) {
          ...
          error.message.includes('The query extension must extend the')
        ) {
          errorMessageText =
            i18n.t('Query extension mismatch: ') + error.message
        } else if (error.message.includes('Syntax Error')) {
          errorMessageText = i18n.t(
            'Query extension contains invalid GraphQL syntax',
          )
        } else {
        ...
        toast.error(i18n.t('Query extension error'), {
          description:
            errorMessageText +
            '. ' +
            i18n.t('The page will continue with the default query.'),
        })
      }
    }
