...
globalRegistry.register(
  ...
  new Map<string, DashboardActionBarItem[]>(),
)
globalRegistry.register(
  'dashboardPageBlockRegistry',
  new Map<string, DashboardPageBlockDefinition[]>(),
)
...
export function registerDashboardPageBlock(
  block: DashboardPageBlockDefinition,
) {
  globalRegistry.set('dashboardPageBlockRegistry', (map) => {
    map.set(block.location.pageId, [
      ...(map.get(block.location.pageId) ?? []),
      block,
    ])
    return map
  })
