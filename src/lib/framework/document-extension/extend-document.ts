import type { Variables } from '@/graphql/api.js'
...
>(defaultDocument: T, template: TemplateStringsArray, ...values: any[]): T
export function extendDocument<
  T extends TypedDocumentNode,
  V extends Variables = Variables,
>(defaultDocument: T, sdl: string | DocumentNode): T
export function extendDocument<
  T extends TypedDocumentNode,
  V extends Variables = Variables,
>(
...
function mergeDocuments(
  ...
  extensionDocument: DocumentNode,
): DocumentNode {
  ...
  const extensionOperations = extensionDocument.definitions.filter(
  ...
  const extensionFragments =
    extensionDocument.definitions.filter(isFragmentDefinition)
  ...
  for (const extensionOp of extensionOperations) {
    ...
    if (baseFieldNode.selectionSet && extensionField.selectionSet) {
      mergeSelectionSets(
        baseFieldNode.selectionSet,
        extensionField.selectionSet,
      )
    }
  }
...
function mergeSelectionSets(
  ...
  for (const extensionField of extensionFields) {
    const existingField = baseFields.find(
      (field) => field.name.value === extensionField.name.value,
    )
...
function isFragmentDefinition(
  value: DefinitionNode,
): value is FragmentDefinitionNode {
...
export function gqlExtend(strings: TemplateStringsArray, ...values: any[]) {
  return (defaultDocument: DocumentNode) =>
    extendDocument(defaultDocument, strings, ...values)
}
