...
describe('form-schema-tools', () => {
  ...
  describe('createFormSchemaFromFields - custom fields (root context)', () => {
    it('should apply string pattern validation for root custom fields', () => {
      ...
      const customFields = [
        createMockCustomField('sku', 'string', {
          pattern: '^[A-Z]{2}-\\d{4}$',
        }),
      ]
    ...
    it('should apply int min/max validation for root custom fields', () => {
      ...
      const customFields = [
        createMockCustomField('quantity', 'int', { intMin: 1, intMax: 100 }),
      ]
    ...
    it('should apply datetime min/max validation for root custom fields', () => {
      ...
      const validDataString = {
        customFields: { releaseDate: '2023-06-15T12:00:00.000Z' },
      }
      expect(() => schema.parse(validDataString)).not.toThrow()

      // Test with Date object
      const validDataDate = {
        customFields: { releaseDate: new Date('2023-06-15T12:00:00.000Z') },
      }
      expect(() => schema.parse(validDataDate)).not.toThrow()

      const beforeMinData = {
        customFields: { releaseDate: '2019-12-31T23:59:59.999Z' },
      }
      expect(() => schema.parse(beforeMinData)).toThrow()

      const afterMaxData = {
        customFields: { releaseDate: '2026-01-01T00:00:00.000Z' },
      }
    ...
    it('should handle list custom fields', () => {
      ...
      const customFields = [
        createMockCustomField('tags', 'string', { list: true }),
      ]
    ...
    it('should handle nullable custom fields', () => {
      ...
      const customFields = [
        createMockCustomField('optionalField', 'string', { nullable: true }),
      ]
  ...
  describe('createFormSchemaFromFields - custom fields (translation context)', () => {
    it('should handle localeString custom fields in translation context', () => {
      ...
      const customFields = [
        createMockCustomField('description', 'localeString', {
          pattern: '^[A-Za-z\\s]+$',
        }),
      ]
  ...
  describe('createFormSchemaFromFields - error messages', () => {
    ...
    it('should provide clear error messages for min validation', () => {
      ...
      const customFields = [
        createMockCustomField('quantity', 'int', { intMin: 1 }),
      ]
    ...
    it('should provide clear error messages for max validation', () => {
      ...
      const customFields = [
        createMockCustomField('quantity', 'int', { intMax: 100 }),
      ]
    ...
    it('should provide clear error messages for datetime validation', () => {
      ...
      try {
        schema.parse({
          customFields: { releaseDate: new Date('2019-12-31T23:59:59.999Z') },
        })
        expect.fail('Should have thrown validation error')
      } catch (error: any) {
        expect(error.issues).toBeDefined()
