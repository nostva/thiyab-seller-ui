import type { FieldInfo } from '../document-introspection/get-document-structure.js'
 ...
 */
export function transformRelationFields<E extends Record<string, any>>(
  fields: FieldInfo[],
  entity: E,
): E {
  ...
  const customFieldsInfo = fields.find(
    (field) => field.name === 'customFields' && field.typeInfo,
  )
  ...
  const idTypeCustomFields = customFieldsInfo.typeInfo.filter(
    (field) => field.type === 'ID',
  )
 ...
 */
export function removeEmptyIdFields<T extends Record<string, any>>(
  values: T,
  fields: FieldInfo[],
): T {
  if (!values) {
