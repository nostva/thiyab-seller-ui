...
import type { DashboardCustomFormComponent } from '../extension-api/types/form-components.js'
import { globalRegistry } from '../registry/global-registry.js'

import type { CustomFormComponentInputProps } from './custom-form-component.js'
...
export function addCustomFormComponent({
  id,
  component,
  ...
  if (customFormComponents.has(id)) {
    ...
    console.warn(
      `Custom form component with id "${id}" is already registered and will be overwritten.`,
    )
  }
...
export function addDetailQueryDocument(pageId: string, document: DocumentNode) {
  const listQueryDocumentRegistry = globalRegistry.get(
    'detailQueryDocumentRegistry',
  )
