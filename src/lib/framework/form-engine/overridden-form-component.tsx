import {
  ...
  type DataInputComponent,
  useComponentRegistry,
} from '@/framework/component-registry/component-registry.js'
import { generateInputComponentKey } from '@/framework/extension-api/input-component-extensions.js'
 ...
 * ```
 */
export function OverriddenFormComponent({
  fieldName,
  field,
  children,
}: Readonly<OverriddenFormComponent>) {
  const page = usePage()
  ...
  if (page.pageId && pageBlock?.blockId) {
    const customInputComponentKey = generateInputComponentKey(
      page.pageId,
      pageBlock.blockId,
      fieldName,
    )
    DisplayComponent = componentRegistry.getDisplayComponent(
      customInputComponentKey,
    )
    InputComponent = componentRegistry.getInputComponent(
      customInputComponentKey,
    )
  }
  if (DisplayComponent) {
