...
import type { StructCustomFieldConfig } from '@vendure/common/lib/generated-types'
...
type StructFieldConfig = ResultOf<
  typeof structCustomFieldFragment
>['fields'][number]

function mapGraphQLCustomFieldToConfig(
  field: StructFieldConfig,
): CustomFieldConfig {
  const baseConfig = {
    name: field.name,
    type: field.type,
 ...
 */
function createDateValidationSchema(
 ...
 */
function createCustomFieldValidationSchema(
  ...
  switch (customField.type) {
    ...
    case 'float':
      zodType = createFloatValidationSchema(
        customField.floatMin,
        customField.floatMax,
      )
      break
    case 'datetime': {
 ...
 */
function createStructFieldSchema(
  structFieldConfig: StructCustomFieldConfig,
): ZodType {
 ...
 */
function applyListAndNullableModifiers(
  zodType: ZodType,
  customField: CustomFieldConfig,
): ZodType {
...
export function createFormSchemaFromFields(
  ...
  for (const field of fields) {
    ...
    if ((isScalar || isEnum) && field.name !== 'customFields') {
      ...
      const isNestedTranslationContext =
        field.name === 'translations' || isTranslationContext
...
export function getDefaultValuesFromFields(
  ...
  for (const field of fields) {
    if (field.typeInfo) {
      const nestedObjectDefaults = getDefaultValuesFromFields(
        field.typeInfo,
        defaultLanguageCode,
      )
      if (field.list) {
      ...
      defaultValues[field.name] = getDefaultValueFromField(
        field,
        defaultLanguageCode,
      )
    }
...
export function getDefaultValueFromField(
  field: FieldInfo,
  defaultLanguageCode?: string,
) {
  if (field.list) {
...
export function getZodTypeFromField(
  field: FieldInfo,
  customFieldConfigs?: CustomFieldConfig[],
): ZodTypeAny {
  let zodType: ZodType
