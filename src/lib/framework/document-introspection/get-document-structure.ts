...
import type {
  ...
  SelectionSetNode,
  TypeNode,
} from 'graphql/language/ast.js'
import { schemaInfo } from '@/graphql/schema-info'
...
>(documentNode: T, path: P): FieldInfo[] {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (!fieldNode.selectionSet) {
      throw new Error(
        `Field "${pathSegment}" has no selection set but path continues`,
      )
    }
...
function findMutationOperation(
  documentNode: DocumentNode,
): OperationDefinitionNode {
  const operationDefinition = documentNode.definitions.find(
    (def): def is OperationDefinitionNode =>
...
function processPaginatedList(
  ...
  const itemsField = field.selectionSet?.selections.find(
    (selection) =>
      selection.kind === 'Field' && selection.name.value === 'items',
  ) as FieldNode
  if (!itemsField) {
    return
  }
  ...
  const isPaginatedList =
    typeFields.hasOwnProperty('items') &&
    typeFields.hasOwnProperty('totalItems')
  if (!isPaginatedList) {
...
export function getOperationVariablesFields<
  T extends TypedDocumentNode<any, any>,
>(documentNode: T, varName?: keyof VariablesOf<T>): FieldInfo[] {
  const fields: FieldInfo[] = []
  ...
  for (const selection of selectionSet.selections) {
    if (selection.kind === 'Field') {
      ...
      if (fieldNode.selectionSet && !fieldInfo.isScalar) {
        const result = findPaginatedListPath(
...
function getInputTypeFields(name: string): FieldInfo[] {
  ...
  return Object.entries(inputType).map(
