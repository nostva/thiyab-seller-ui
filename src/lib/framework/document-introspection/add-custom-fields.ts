...
import type { CustomFieldConfig } from '@vendure/common/lib/generated-types'
...
const memoizationCache = new WeakMap<
  DocumentNode,
  Map<string, TypedDocumentNode<any, any>>
>()
  ...
  for (const target of targetNodes) {
    ...
    if (customFieldsForType && customFieldsForType.length) {
      ...
      const existingCustomFieldsField = target.selectionSet.selections.find(
        (selection) =>
          isFieldNode(selection) && selection.name.value === 'customFields',
      ) as FieldNode | undefined
      const selectionNodes: SelectionNode[] = customFieldsForType
        .filter(
          (field) =>
            !options?.includeCustomFields ||
            options?.includeCustomFields.includes(field.name),
        )
        .map(
          (customField) =>
            ({
              ...
              ...(customField.type === 'struct'
                ? {
                    selectionSet: {
                      kind: Kind.SELECTION_SET,
                      selections: (
                        customField as StructCustomFieldFragment
                      ).fields.map((f) => ({
                        kind: Kind.FIELD,
