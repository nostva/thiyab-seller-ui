...
> {
  ...
  ) => VariablesOf<U>[VarNameUpdate]
  transformCreateInput?: (
    input: VariablesOf<C>[VarNameCreate],
  ) => VariablesOf<C>[VarNameCreate]
   ...
   */
  onSuccess?: (
    entity: ResultOf<C>[keyof ResultOf<C>] | ResultOf<U>[keyof ResultOf<U>],
  ) => void
  /**
   * @description
   * The function to call when the update is successful.
...
> = 'translations' extends keyof NonNullable<ResultOf<T>[EntityField]>
  ? Array<
      NonNullable<ResultOf<T>[EntityField]>['translations'][number] & {
        customFields?: any
      }
    >
  : undefined
