...
export function ManageLanguagesDialog({
  ...
  // Permission checks
  const canReadGlobalSettings =
    hasPermissions(['ReadSettings']) || hasPermissions(['ReadGlobalSettings'])
  const canUpdateGlobalSettings =
    hasPermissions(['UpdateSettings']) ||
    hasPermissions(['UpdateGlobalSettings'])
  ...
  const [channelDefaultLanguage, setChannelDefaultLanguage] =
    useState<string>('')
  ...
  useEffect(() => {
    if (open && globalSettingsData) {
      setGlobalLanguages(
        globalSettingsData.globalSettings.availableLanguages || [],
      )
    }
  ...
  const handleGlobalLanguagesChange = (newLanguages: string[]) => {
    ...
    const updatedChannelLanguages = channelLanguages.filter((lang) =>
  ...
  const handleSave = async () => {
    ...
    if (canUpdateGlobalSettings && globalSettingsData) {
      const currentGlobalLanguages =
        globalSettingsData.globalSettings.availableLanguages || []
      if (
        JSON.stringify(currentGlobalLanguages.sort()) !==
        JSON.stringify(globalLanguages.sort())
      ) {
        promises.push(
    ...
    if (canUpdateChannel && displayChannel) {
      const currentChannelLanguages =
        displayChannel.availableLanguageCodes || []
      const currentChannelDefault = displayChannel.defaultLanguageCode || ''

      const languagesChanged =
        JSON.stringify(currentChannelLanguages.sort()) !==
        JSON.stringify(channelLanguages.sort())
  ...
  const hasChanges = () => {
    if (globalSettingsData && canUpdateGlobalSettings) {
      const currentGlobal =
        globalSettingsData.globalSettings.availableLanguages || []
      if (
        JSON.stringify(currentGlobal.sort()) !==
        JSON.stringify(globalLanguages.sort())
      ) {
        return true
      }
    ...
    if (displayChannel && canUpdateChannel) {
      ...
      return (
        JSON.stringify(currentChannelLangs.sort()) !==
          JSON.stringify(channelLanguages.sort()) ||
  ...
  const isLoading =
    updateGlobalSettingsMutation.isPending || updateChannelMutation.isPending
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            <Trans>
              Configure available languages for your store and channels
            </Trans>
          </DialogDescription>
        </DialogHeader>
            ...
            {!canReadGlobalSettings ? (
              <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
                ...
                <span className="text-sm text-muted-foreground">
                  <Trans>
                    You don't have permission to view global language settings
                  </Trans>
                </span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  <Trans>
                    These languages will be available for all channels to use
                  </Trans>
                </p>
              </div>
            )}
          ...
          <div>
            ...
            {!canReadChannel ? (
              <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
                ...
                <span className="text-sm text-muted-foreground">
                  <Trans>
                    You don't have permission to view channel settings
                  </Trans>
                </span>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div
                    className={
                      !canUpdateChannel ? 'pointer-events-none opacity-50' : ''
                    }
                  >
                    <LanguageSelector
                  ...
                  {globalLanguages.length === 0 ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      <Trans>
                        Select from globally available languages for this
                        channel
                      </Trans>
                    </p>
                  )}
