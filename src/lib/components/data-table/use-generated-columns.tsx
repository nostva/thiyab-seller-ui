...
import {
  ...
  type AllItemFieldKeys,
  type CustomizeColumnConfig,
  type FacetedFilterConfig,
  type PaginatedListItemFields,
  type RowAction,
  usePaginatedList,
...
function DeleteMutationRowAction({
  ...
  const hasIdsParameter = mutationVariables.some(
    (field) => field.name === 'ids',
  )
  ...
  const { mutate: deleteMutationFn } = useMutation({
    ...
    onSuccess: (result: {
      ...
      const resultToCheck = Array.isArray(unwrappedResult)
        ? unwrappedResult[0]
        : unwrappedResult
