...
function EntityLabel({
  ...
  return (
    <div
      ...
      title={tooltipText || `${title} (${subtitle})`}
    >
      <div
...
const createEntityConfigs = (i18n: any) => ({
  ...
  Customer: createRelationSelectorConfig({
    ...
    label: (item: any) => (
      <EntityLabel
        ...
        subtitle={[item.emailAddress, item.phoneNumber]
          .filter(Boolean)
          .join(' • ')}
        placeholderLetter={
        ...
        rounded
        statusIndicator={
          <StatusBadge condition={!item.user?.verified} text="Unverified" />
        }
  ...
  ProductVariant: createRelationSelectorConfig({
    ...
    listQuery: graphql(`
      query GetProductVariantsForRelationSelector(
    ...
    label: (item: any) => (
      <EntityLabel
        ...
        placeholderLetter="V"
        statusIndicator={
          <StatusBadge condition={!item.enabled} text="Disabled" />
        }
  ...
  Collection: createRelationSelectorConfig({
    ...
    label: (item: any) => (
      <EntityLabel
        ...
        placeholderLetter="C"
        statusIndicator={
          <StatusBadge condition={item.isPrivate} text="Private" />
        }
        tooltipText={`${item.name} (${item.slug})`}
      />
    ),
  ...
  Asset: createRelationSelectorConfig({
    ...
    label: (item: any) => {
      ...
      const fileSize = item.fileSize
        ...
        : ''
      const subtitle = [item.mimeType, dimensions, fileSize]
        .filter(Boolean)
        .join(' • ')
      const tooltipDetails = [item.mimeType, dimensions, fileSize]
        .filter(Boolean)
        .join(', ')
  ...
  ShippingMethod: createRelationSelectorConfig({
    ...
    listQuery: graphql(`
      query GetShippingMethodsForRelationSelector(
  ...
  PaymentMethod: createRelationSelectorConfig({
    ...
    listQuery: graphql(`
      query GetPaymentMethodsForRelationSelector(
  ...
  CustomerGroup: createRelationSelectorConfig({
    ...
    listQuery: graphql(`
      query GetCustomerGroupsForRelationSelector(
    ...
    label: (item: any) => {
      ...
      return (
        <EntityLabel
          ...
          tooltipText={
            item.couponCode ? `${item.name} (${item.couponCode})` : item.name
          }
        />
      )
    },
