    ...
    if (items.length > 0 && selectedIds.size > 0) {
      ...
      if (newSelectedItems.length > 0) {
        setSelectedItems((prevItems) => {
          ...
          const uniqueNewItems = newSelectedItems.filter(
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] md:max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle>
            <Trans>
              {mode === 'product' ? 'Select Products' : 'Select Variants'}
            </Trans>
          </DialogTitle>
        </DialogHeader>
...
export const ProductMultiInput: DataInputComponent = ({
  value,
  onChange,
  ...props
}) => {
  ...
  const mode =
    (props as any)?.selectionMode === 'variant' ? 'variant' : 'product'
  ...
  const buttonText =
    selectedIds.length > 0
      ? `Selected ${selectedIds.length} ${itemType}`
      : `Select ${itemType}`
