...
function convertToItemsWithIds(
  values: any[],
  existingItems?: ListItemWithId[],
): ListItemWithId[] {
  ...
  return values.map((value, index) => {
    ...
    if (
      existingItem &&
      JSON.stringify(existingItem.value) === JSON.stringify(value)
    ) {
      return existingItem
    }
...
export function CustomFieldListInput({
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(convertToFlatArray(newItems)) !==
      JSON.stringify(convertToFlatArray(itemsWithIds))
    ) {
  ...
  const itemIds = useMemo(
    () => itemsWithIds.map((item) => item._id),
    [itemsWithIds],
  )
  ...
  const handleItemChange = useCallback(
    (id: string, value: any) => {
      const newItemsWithIds = itemsWithIds.map((item) =>
