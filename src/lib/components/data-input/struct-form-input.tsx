        ...
        {fieldDef.fields.map((structField) => (
          <React.Fragment key={structField.name}>
            ...
            <dd className="text-sm text-foreground py-2">
              {formatFieldValue(
                watchedStructValue[structField.name],
                structField,
              )}
            </dd>
          </React.Fragment>
...
export function StructFormInput({
  ...
  const formatFieldValue = (value: any, structField: StructField) => {
    ...
    switch (structField.type) {
      case 'boolean':
        return (
          <span
            className={`inline-flex items-center ${value ? 'text-green-600' : 'text-red-500'}`}
          >
            {value ? (
  ...
  const renderStructFieldInput = (
    ...
    const renderSingleStructInput = (
      singleField: ControllerRenderProps<any, any>,
    ) => {
      switch (structField.type) {
        case 'string': {
    ...
    if (isList) {
      ...
      const needsFullWidth =
        structField.type === 'text' || structField.type === 'localeText'

      return (
        <CustomFieldListInput
          field={inputField}
          disabled={isReadonly}
          renderInput={(index, listItemField) =>
            renderSingleStructInput(listItemField)
          }
