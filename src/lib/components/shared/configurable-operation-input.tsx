...
import { type ConfigurableOperationInput as ConfigurableOperationInputType } from '@vendure/common/lib/generated-types'

import { Button } from '../ui/button'
import { Form, FormControl, FormField, FormItem, FormLabel } from '../ui/form'
import { ConfigurableOperationArgInput } from './configurable-operation-arg-input'
...
export function ConfigurableOperationInput({
  ...
  const handleInputChange = (name: string, inputValue: string) => {
    ...
    const newVal: ConfigurableOperationInputType = {
      ...value,
...
export function interpolateDescription(
  ...
  const interpolated = templateString.replace(
    ...
    (substring: string, argName: string) => {
      ...
      let formatted = value
      const argDef = operation.args.find(
        (arg: any) => arg.name === normalizedArgName,
      )
      if (
        argDef &&
        argDef.type === 'int' &&
