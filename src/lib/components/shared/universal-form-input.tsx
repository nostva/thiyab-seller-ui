...
import { CustomFieldListInput } from '@/components/data-input/custom-field-list-input.js'
import { StructFormInput } from '@/components/data-input/struct-form-input.js'
import {
  CustomFormComponent,
  type CustomFormComponentInputProps,
} from '@/framework/form-engine/custom-form-component.js'
...
import type { ValueMode } from './value-transformers.js'
...
export function UniversalFormInput({
  ...
  if (uiComponent && valueMode === 'native') {
    ...
    return (
      <CustomFormComponent fieldDef={fieldDef as any} fieldProps={fieldProps} />
    )
  }

  // Handle direct component mapping (config args mode)
  if (uiComponent && valueMode === 'json-string') {
    const DirectComponent = getDirectFormComponent(
      uiComponent as DefaultFormComponentId,
    )
    if (DirectComponent) {
      return (
        <DirectComponent
        ...
        <CustomFieldListInput
          ...
          renderInput={(index, inputField) => (
            <StructFormInput
              ...
              control={control}
              getTranslation={getTranslation ?? (() => undefined)}
            />
          )}
          defaultValue={{}}
          isFullWidth={true}
