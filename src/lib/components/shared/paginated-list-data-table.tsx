...
import { useListQueryFields } from '@/framework/document-introspection/hooks'
...
import { DisplayComponent } from '@/framework/component-registry/dynamic-component'
...
type IsPaginatedList<T> = T extends { items: any[]; totalItems: number }
  ? true
  : false
...
type Level2PaginatedLists<T> = T extends object
  ? {
      [K1 in StringKeys<T>]: NonNullable<T[K1]> extends object
        ? {
            [K2 in StringKeys<NonNullable<T[K1]>>]: NonNullable<
              NonNullable<T[K1]>[K2]
            > extends object
              ? IsPaginatedList<
...
export type PaginatedListPaths<T extends TypedDocumentNode<any, any>> =
  FindPaginatedListPaths<ResultOf<T>> extends infer Paths
    ? Paths extends ''
      ? never
      : Paths
    : never
...
export type CustomFieldKeysOfItem<Item> = Item extends {
...
export type CustomizeColumnConfig<T extends TypedDocumentNode<any, any>> = {
  [Key in AllItemFieldKeys<T>]?: Partial<
    ColumnDef<PaginatedListItemFields<T>, any>
  >
}
...
export const PaginatedListContext = React.createContext<
  PaginatedListContext | undefined
>(undefined)
...
export function usePaginatedList() {
  ...
  if (!context) {
    throw new Error(
      'usePaginatedList must be used within a PaginatedListDataTable',
    )
  }
  return context
}
...
> {
  ...
  transformData?: (
    data: PaginatedListItemFields<T>[],
  ) => PaginatedListItemFields<T>[]
...
>({
  ...
  const queryKey = transformQueryKey
    ? transformQueryKey(defaultQueryKey)
    : defaultQueryKey
  ...
  const { columns, customFieldColumnNames } = useMemo(() => {
      ...
      ...fields // Filter out custom fields
        .filter(
          (field) =>
            ...
            !field.type.endsWith('CustomFields'),
        )
        .map((field) => ({ fieldInfo: field, isCustomField: false })),
    )

    const customFieldColumn = fields.find(
      (field) => field.name === 'customFields',
    )
    if (customFieldColumn && customFieldColumn.type !== 'JSON') {
      const customFieldFields = getTypeFieldInfo(customFieldColumn.type)
      columnConfigs.push(
        ...customFieldFields.map((field) => ({
          fieldInfo: field,
          isCustomField: true,
        })),
      )
      customFieldColumnNames.push(
        ...customFieldFields.map((field) => field.name),
      )
    }

    const queryBasedColumns = columnConfigs.map(
      ({ fieldInfo, isCustomField }) => {
        const customConfig =
          customizeColumns?.[
            fieldInfo.name as unknown as AllItemFieldKeys<T>
          ] ?? {}
        const { header, ...customConfigRest } = customConfig
        const enableColumnFilter =
          fieldInfo.isScalar && !facetedFilters?.[fieldInfo.name]
        ...
        return columnHelper.accessor(fieldInfo.name as any, {
          ...
          cell: ({ cell, row }) => {
            ...
            const value =
              cellValue ??
              (isCustomField
                ? row.original?.customFields?.[fieldInfo.name]
                : undefined)
            ...
            if (fieldInfo.type === 'Boolean') {
              if (cell.column.id === 'enabled') {
                return (
                  <DisplayComponent id="vendure:booleanBadge" value={value} />
                )
              } else {
                return (
                  <DisplayComponent
                    id="vendure:booleanCheckbox"
                    value={value}
                  />
                )
              }
    ...
    if (defaultColumnOrder) {
      ...
      const orderedColumns = finalColumns
        .filter(
          (column) =>
            column.id && defaultColumnOrder.includes(column.id as any),
        )
        .sort(
          (a, b) =>
            defaultColumnOrder.indexOf(a.id as any) -
            defaultColumnOrder.indexOf(b.id as any),
        )
  ...
  return (
    <PaginatedListContext.Provider value={{ refetchPaginatedList }}>
      <DataTable
        ...
        onSearchTermChange={
          onSearchTermChange ? (term) => setSearchTerm(term) : undefined
        }
...
function DeleteMutationRowAction({
  ...
  const { mutate: deleteMutationFn } = useMutation({
    ...
    onSuccess: (result: {
      [key: string]: { result: 'DELETED' | 'NOT_DELETED'; message: string }
    }) => {
...
function getColumnVisibility(
  ...
  const allDefaultsTrue =
    defaultVisibility &&
    Object.values(defaultVisibility).every((v) => v === true)
  const allDefaultsFalse =
    defaultVisibility &&
    Object.values(defaultVisibility).every((v) => v === false)
  return {
    id: false,
    createdAt: false,
    updatedAt: false,
