...
export function HistoryEntry({
  ...
  const getIconColor = (type: string) => {
    ...
    if (
      type === 'ORDER_PAYMENT_TRANSITION' &&
      (entry.data.to === 'Declined' || entry.data.to === 'Cancelled')
    ) {
      return 'bg-destructive text-destructive-foreground'
    }
  ...
  return (
    <div key={entry.id} className="relative group">
      ...
      >
        <div
          className={cn(
            `relative z-10 flex-shrink-0 ${isNoteEntry ? 'ml-2' : ''}`,
            isPrimary ? '-ml-1' : '',
          )}
        >
          <div
          ...
          >
            <div
              className={isPrimary ? 'text-current' : 'text-current scale-75'}
            >
              {timelineIcon}
            </div>
          </div>
              ...
              <div className="mt-1">
                {entry.type === 'ORDER_NOTE' ? (
                  <div className={`space-y-${isPrimary ? '2' : '1'}`}>
                    <p
                      className={`${isPrimary ? 'text-sm' : 'text-xs'} text-foreground`}
                    >
                      {entry.data.note}
                    </p>
