...
import { type ValueMode, transformValue } from './value-transformers.js'
...
function renderNumericInput({
  ...
  if (shouldUseAffixedInput) {
    const numericValue =
      transformedValue !== undefined && transformedValue !== ''
        ? typeof transformedValue === 'number'
          ? transformedValue
          : parseFloat(transformedValue) || ''
        : ''
...
function renderTextareaInput({
  ...
  if (
    valueMode !== 'json-string' ||
    fieldDef.ui?.component !== 'textarea-form-input'
  )
    return null
...
export function UniversalInputComponent({
  ...
  const handleNumericChange = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      handleChange(
        isNaN(val) ? (valueMode === 'json-string' ? '' : undefined) : val,
      )
    },
    [handleChange, valueMode],
  )

  const handleRegularNumericChange = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const val = e.target.valueAsNumber
